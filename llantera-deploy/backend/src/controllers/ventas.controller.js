...
const generarFolio = async (client) => {
  ...
  const { rows } = await client.query(
    `SELECT COUNT(*) FROM ventas WHERE EXTRACT(YEAR FROM created_at) = $1`, [año]
  );
  ...
  try {
    ...
    for (const item of items) {
      const { rows: [prod] } = await client.query(
        'SELECT id, nombre, precio_venta, stock_actual, es_servicio FROM productos WHERE id = $1 FOR UPDATE',
        [item.producto_id]
      );
      ...
      if (!item.producto.es_servicio) {
        ...
        await client.query(
          `INSERT INTO movimientos_inventario
             (producto_id, tipo, cantidad, stock_antes, stock_despues, referencia_tipo, referencia_id, usuario_id)
           VALUES ($1,'salida',$2,$3,$4,'venta',$5,$6)`,
          [item.producto.id, item.cantidad, item.producto.stock_actual, stock_nuevo, venta.id, req.user.id]
        );
      }
    }
    ...
    if (estado === 'pendiente' && cliente_id) {
      await client.query(
        `INSERT INTO cuentas_cobrar (cliente_id, venta_id, monto_total, monto_pagado, fecha_vencimiento)
         VALUES ($1,$2,$3,$4, CURRENT_DATE + INTERVAL '30 days')`,
        [cliente_id, venta.id, total, monto_pagado || 0]
      );
      await client.query(
        'UPDATE clientes SET saldo_pendiente = saldo_pendiente + $1 WHERE id = $2',
        [total - (monto_pagado || 0), cliente_id]
      );
    }
