 ...
 *
 * SCHEMA REAL (obtenido con dump-schema.js — Railway 2026-07-21):
 * Columnas GENERATED ALWAYS (NUNCA insertar):
 *   - ventas_detalle.subtotal   = ((cantidad * precio_unitario) - descuento)
 *   - compras_detalle.subtotal  = (cantidad * costo_unitario)
 *   - ordenes_detalle.subtotal  = ((cantidad * precio_unitario) - descuento)
 *   - cotizaciones_detalle.subtotal = (cantidad * precio_unitario)
 *   - cuentas_cobrar.saldo      = (monto_total - monto_pagado)
 *   - lotes_detalle.cantidad_efectiva = (cantidad_recibida - cantidad_defectuosa)
 *
 * Columnas de ventas que SÍ son escribibles (NO GENERATED):
 *   ventas.subtotal, ventas.total, ventas.iva, ventas.descuento
 */
...
try {
  ...
  let { rows:[uAdmin] } = await client.query(`SELECT id FROM usuarios WHERE email='admin@demo.com' LIMIT 1`);
  let adminId;
  if (!uAdmin) {
    adminId = uuid();
    await client.query(
  ...
  let { rows:[uVend] } = await client.query(`SELECT id FROM usuarios WHERE email='vendedor@demo.com' LIMIT 1`);
  if (!uVend) {
    await client.query(
      `INSERT INTO usuarios (id,negocio_id,nombre,email,password_hash,rol_id,activo)
  ...
  // ── 4. Categorías (tabla global — sin negocio_id) ─────────────────────────
  const catMap = {};
  for (const tipo of ['llanta','refaccion','consumible','servicio']) {
    const nombre = tipo.charAt(0).toUpperCase() + tipo.slice(1);
    let { rows:[cat] } = await client.query(`SELECT id FROM categorias WHERE LOWER(TRIM(nombre))=$1 LIMIT 1`,[tipo]);
    if (!cat) {
      const { rows:[nc] } = await client.query(`INSERT INTO categorias (nombre,tipo) VALUES ($1,$2) RETURNING id`,[nombre,tipo]);
      cat = nc;
    }
    catMap[tipo] = cat.id;
  }
  ...
  // ── 5. Marcas ─────────────────────────────────────────────────────────────
  for (const m of ['HANKOOK','GOODYEAR','BRIDGESTONE','MICHELIN','CONTINENTAL','YOKOHAMA','REMA','AIRTEC','GENERICO']) {
    await client.query(
  ...
  for (const p of PRODUCTOS) {
    ...
    await client.query(
      ...
      [pid,NID,p.nombre,p.medida||null,p.marca||null,sku,p.pv,p.pc,p.stock,rndN(2,5),catMap[p.tipo]||null,p.svc||false]
    );
  ...
  for (const c of CLIENTES) {
    let { rows:[ex] } = await client.query(`SELECT id FROM clientes WHERE negocio_id=$1 AND nombre=$2 LIMIT 1`,[NID,c.nombre]);
    if (!ex) {
      const cid = uuid();
      await client.query(
      ...
      clientIds.push(cid); cliCreados++;
    } else {
      clientIds.push(ex.id);
    }
  }
  console.log(`✅  ${cliCreados} cliente(s) creado(s)`);

  // ── 8. Ventas ─────────────────────────────────────────────────────────────
  // SCHEMA REAL:
  //   ventas.subtotal, total, iva, descuento → columnas NORMALES (escribibles)
  //   ventas_detalle.subtotal                → GENERATED ALWAYS AS ((cantidad*precio_unitario)-descuento)
  //                                            → NO SE INSERTA, PostgreSQL lo calcula
  ...
  if (parseInt(cv.cnt) === 0) {
    const vendibles = [
      ...
      { key:'PARCHE_4 GRANDE',             pv:25   },
      { key:'PARCHE_3 MEDIANO',            pv:18   },
    ...
    for (const dias of patronDias) {
      ...
      // INSERT ventas — subtotal/total/iva/descuento son columnas NORMALES (escribibles)
      // Parámetros separados por tipo para evitar "inconsistent types":
      //   $7 = subtotal (NUMERIC)
      //   $8 = metodo   (TEXT)
      //   $9 = subtotal (NUMERIC) — reutilizamos el valor para total y monto_pagado
      //                              pero como parámetros SEPARADOS para claridad de tipos
      await client.query(
        `INSERT INTO ventas
           (id,negocio_id,folio,cliente_id,usuario_id,fecha,
            subtotal,descuento,iva,total,metodo_pago,monto_pagado,cambio,estado)
         VALUES ($1,$2,$3,$4,$5,$6,$7,0,0,$7,$8,$9,0,'pagada')
         ON CONFLICT DO NOTHING`,
        [ventaId, NID, folio, cliId, adminId, fecha.toISOString(),
    ...
    console.log(`✅  ${ventasCreadas} venta(s) creada(s)`);
  } else {
  ...
  if (parseInt(cg.cnt) === 0) {
    ...
    for (const g of GASTOS) {
      if (!cgMap[g.cat]) {
        let { rows:[cat] } = await client.query(`SELECT id FROM categorias_gasto WHERE LOWER(nombre)=$1 LIMIT 1`,[g.cat.toLowerCase()]);
        if (!cat) {
          const { rows:[nc] } = await client.query(`INSERT INTO categorias_gasto (nombre) VALUES ($1) RETURNING id`,[g.cat]);
          cat = nc;
        }
        cgMap[g.cat] = cat.id;
      }
    ...
    console.log(`✅  ${GASTOS.length} gasto(s) creado(s)`);
  } else {
